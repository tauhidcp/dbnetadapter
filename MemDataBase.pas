...
type
  ...
  protected
    ...
    procedure InternalGotoBookmark(aBookmark: TBookmarkPointerType);
...
{$ELSE}
  Result := FActive and (PInteger(aBookmark)^ > Low(integer)) and
    (PInteger(aBookmark)^ <= FLastID);
{$ENDIF}
end;
...
var
  ...
  Accept: Boolean;
begin
  Rec := FindRecordID(PInteger(aBookmark)^);
  if Rec <> nil then
  begin
    Accept := True;
    SavePos := FRecordPos;
